...
import { MessageSquare, User, Bot, Send, Image, Paperclip, Sparkles } from "lucide-react";
...
const ChatbotPlaceholder = () => {
  const [messages, setMessages] = useState<{ text: string; isUser: boolean; timestamp: Date }[]>([
    {
      text: "Hello! I'm your EO Data Center AI Assistant powered by Mistral. Ask me anything about your data!",
      isUser: false,
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { text: "Sorry, something went wrong. Try again!", isUser: false, timestamp: new Date() },
      ]);
    } finally {
      setIsLoading(false);
    }
